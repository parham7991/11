...
export const metadata: Metadata = {
  ...
  description: 'راهنمای نصب نسخه PWA آفلند برای دسترسی سریع به فروشگاه، اسمبل آنلاین هوشمند، سبد خرید، جستجو و مجله.',
};
...
const features = [
  ...
  ['کش هوشمند', 'صفحات و تصاویر عمومی با استراتژی امن کش می‌شوند، اما سبد خرید و اطلاعات کاربر همیشه تازه می‌ماند.'],
  ['حالت آفلاین', 'اگر اینترنت قطع شد، صفحه آفلاین شیک با مسیرهای اصلی نمایش داده می‌شود.'],
  ['به‌روزرسانی نرم', 'وقتی نسخه جدید آماده باشد، کاربر پیام به‌روزرسانی می‌بیند و با یک کلیک نسخه تازه را می‌گیرد.'],
  ['سازگار با iOS/Android', 'آیکون استاندارد، maskable icon و Apple Web App (apple-touch-icon) تنظیم شده‌اند تا روی iOS و Android مثل اپ باز شود.'],
];
      ...
      <section className="mx-auto max-w-5xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(20,35,80,.10)]">
        <div className="bg-gradient-to-br from-[#386bf9] via-[#6f3cf5] to-[#ec4899] px-6 py-12 text-white md:px-12">
          <div className="mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-white/20 ring-1 ring-white/30">
            <svg viewBox="0 0 24 24" className="h-9 w-9" fill="none"><rect x="7" y="7" width="10" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.8"/><rect x="10" y="10" width="4" height="4" rx=".5" stroke="currentColor" strokeWidth="1.8"/><path d="M9 4v3M12 4v3M15 4v3M9 17v3M12 17v3M15 17v3M4 9h3M4 12h3M4 15h3M17 9h3M17 12h3M17 15h3" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/></svg>
          </div>
          <h1 className="mb-4 text-3xl font-black md:text-5xl">نسخه PWA آفلند</h1>
          <p className="max-w-3xl text-sm leading-8 text-white/90 md:text-base">فروشگاه، اسمبل آنلاین هوشمند، سبد خرید، جستجو، مجله و صفحات اصلی آفلند حالا با تجربه نصب‌شونده، سریع و اپ‌مانند آماده‌اند.</p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href="/assemble-online" className="rounded-2xl bg-white px-5 py-3 text-sm font-bold text-[#386bf9]">شروع اسمبل هوشمند</Link>
            <Link href="/" className="rounded-2xl bg-white/15 px-5 py-3 text-sm font-bold text-white ring-1 ring-white/25">رفتن به فروشگاه</Link>
          </div>
        </div>
        <div className="grid gap-4 p-6 md:grid-cols-2 md:p-10 lg:grid-cols-3">
          {features.map(([title, desc]) => (
            <article key={title} className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
              <h2 className="mb-2 text-base font-black text-slate-900">{title}</h2>
              <p className="text-sm leading-7 text-slate-600">{desc}</p>
            </article>
          ))}
        </div>
        <div className="border-t border-slate-200 bg-white p-6 md:p-10">
          <h2 className="mb-3 text-xl font-black">راهنمای نصب</h2>
