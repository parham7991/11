...
export default function BudgetBreakdown({ parts, budget }: { parts: P[]; budget: number }) {
  const { segments, spend, remaining, total, biggest } = useMemo(() => {
    const spend = parts.reduce(
      (sum, p) => sum + (Number(p.finalPrice) || 0) * Math.max(1, Number(p.quantity || 1)),
      0
    );
    ...
    for (const p of parts) {
      byCat[p.category] =
        (byCat[p.category] || 0) +
        (Number(p.finalPrice) || 0) * Math.max(1, Number(p.quantity || 1));
    }
    const segments = Object.entries(byCat)
      .map(([cat, val]) => ({
        cat,
        val,
        ...(CATEGORY_META[cat] || { label: cat, color: '#64748b' }),
      }))
  ...
  for (const s of drawSegs) {
    ...
    if (end > start)
      arcs.push({
        key: s.key,
        color: s.color,
        d: arcPath(CX, CY, R, start, Math.max(start, end - GAP)),
      });
    acc = end;
  }
  ...
  return (
    <div className="aw-budget-donut">
      <div className="aw-budget-donut__head">
        ...
        <span
          className={`aw-budget-donut__pill${remaining > 0 ? '' : 'aw-budget-donut__pill--full'}`}
        >
      ...
      <div className="aw-budget-donut__body">
        <div className="aw-budget-donut__chart">
          <svg
            ...
            height={SIZE}
            role="img"
            aria-label="نمودار حلقه‌ای تخصیص بودجه"
          >
            {/* حلقهٔ پس‌زمینه */}
            <circle
              cx={CX}
        ...
        <div className="aw-budget-donut__legend">
          {segments.map((s) => {
            ...
            return (
              <div
                key={s.cat}
                className={`aw-budget-donut__leg${biggest?.cat === s.cat ? 'aw-budget-donut__leg--top' : ''}`}
              >
          ...
          {remaining > 0 && (
            <div className="aw-budget-donut__leg aw-budget-donut__leg--remain">
              ...
              <span className="aw-budget-donut__leg-bar">
                <span
                  style={{
                    width: `${Math.round((remaining / total) * 100)}%`,
