...
export default function Collapsible({
  ...
  return (
    <div className={`aw-collapsible${open ? ' aw-collapsible--open' : ''}`}>
      <button type="button" className="aw-collapsible__head" onClick={() => setOpen(o => !o)} aria-expanded={open}>
        {icon && <span className="aw-collapsible__icon">{icon}</span>}
        <span className="aw-collapsible__title">{title}</span>
        <span className="aw-collapsible__chev" aria-hidden="true">{open ? '−' : '+'}</span>
      </button>
