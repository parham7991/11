import React from 'react';
import Link from 'next/link';

interface FakeMenuProps {
  className?: string;
}

const FakeMenu: React.FC<FakeMenuProps> = ({ className = '' }) => {
  const fakeMenus = [
    { title: 'صفحه اصلی', link: '/' },
    { title: 'محصولات', link: '/products' },
    { title: 'درباره ما', link: '/about' },
    { title: 'تماس با ما', link: '/contact' },
  ];

  return (
    <div className={`bg-main ${className}`}>
      <div className="container_page relative hidden h-16 w-full items-center gap-4 lg:flex lg:px-6">
        {/* Category Menu Button */}
        <div className="flex h-10 w-32 items-center gap-3 rounded bg-blue-600 px-3">
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M7.24 2H5.34C3.15 2 2 3.15 2 5.33V7.23C2 9.41 3.15 10.56 5.33 10.56H7.23C9.41 10.56 10.56 9.41 10.56 7.23V5.33C10.57 3.15 9.42 2 7.24 2Z"
              fill="#fff"
            />
            <path
              opacity="0.4"
              d="M18.6699 2H16.7699C14.5899 2 13.4399 3.15 13.4399 5.33V7.23C13.4399 9.41 14.5899 10.56 16.7699 10.56H18.6699C20.8499 10.56 21.9999 9.41 21.9999 7.23V5.33C21.9999 3.15 20.8499 2 18.6699 2Z"
              fill="#fff"
            />
            <path
              d="M18.6699 13.4301H16.7699C14.5899 13.4301 13.4399 14.5801 13.4399 16.7601V18.6601C13.4399 20.8401 14.5899 21.9901 16.7699 21.9901H18.6699C20.8499 21.9901 21.9999 20.8401 21.9999 18.6601V16.7601C21.9999 14.5801 20.8499 13.4301 18.6699 13.4301Z"
              fill="#fff"
            />
            <path
              opacity="0.4"
              d="M7.24 13.4301H5.34C3.15 13.4301 2 14.5801 2 16.7601V18.6601C2 20.8501 3.15 22.0001 5.33 22.0001H7.23C9.41 22.0001 10.56 20.8501 10.56 18.6701V16.7701C10.57 14.5801 9.42 13.4301 7.24 13.4301Z"
              fill="#fff"
            />
          </svg>
          <span className="font-medium text-sm text-white">دسته‌بندی</span>
        </div>

        {/* Menu Items */}
        {fakeMenus.map((item, idx) => (
          <Link
            key={idx}
            prefetch={false}
            className="flex h-10 w-20 items-center justify-center px-3 transition-all duration-500 hover:bg-blue-600"
            href={item.link}
          >
            <span className="text-right font-medium text-[14px] text-white">{item.title}</span>
          </Link>
        ))}
      </div>
    </div>
  );
};

export default FakeMenu;
