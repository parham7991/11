      ...
      if (n.includes(kw.toLowerCase())) {
        return {
          ...
          specs: {
            ...
            integratedGraphics:
              !/f$/i.test(cpu.model) || /5600g|5700g|8600g|8700g/i.test(cpu.model),
          },
  ...
  if (containsAny(n, ['ryzen 5', 'amd ryzen 5', 'r5 ', 'ای ام دی ryzen 5', 'ryzen5'])) {
    return {
      ...
      specs: {
        ...
        tier: 'medium',
        brand: 'AMD',
        integratedGraphics: /5600g|5700g|8600g|8700g/.test(n),
      },
  ...
  if (containsAny(n, ['core i9', 'intel core i9', 'i9-', 'i9 ', 'core i9-'])) {
    return {
      ...
      specs: {
        ...
        tier: 'ultra',
        brand: 'Intel',
        integratedGraphics: !/\d+f\b|kf\b/i.test(n),
      },
  ...
  if (containsAny(n, ['core i3', 'intel core i3', 'i3-', 'i3 '])) {
    return {
      ...
      specs: {
        ...
        tier: 'entry',
        brand: 'Intel',
        integratedGraphics: !/\d+f\b/i.test(n),
      },
...
export function detectGpu(name: string): PartDetection {
  ...
  if (containsAny(n, ['5090', '5080', '5070', '5060'])) {
    return {
      isMatch: true,
      confidence: 88,
      matchedModel: 'RTX 50 Series',
      specs: { tier: 'high', brand: 'NVIDIA' },
      shortSpec: 'RTX 50 Series',
    };
  }
  // RTX 40 series
  if (containsAny(n, ['4090', '4080', '4070', '4060', 'geforce rtx 4'])) {
    return {
      isMatch: true,
      confidence: 88,
      matchedModel: 'RTX 40 Series',
      specs: { tier: 'high', brand: 'NVIDIA' },
      shortSpec: 'RTX 40 Series',
    };
  }
  // RTX 30 series
  if (containsAny(n, ['3090', '3080', '3070', '3060', '3050'])) {
    return {
      isMatch: true,
      confidence: 88,
      matchedModel: 'RTX 30 Series',
      specs: { tier: 'medium', brand: 'NVIDIA' },
      shortSpec: 'RTX 30 Series',
    };
  }
  // GTX 16 series
  if (containsAny(n, ['1660', '1650', '1630', '1050'])) {
    return {
      isMatch: true,
      confidence: 85,
      matchedModel: 'GTX 10/16 Series',
      specs: { tier: 'entry', brand: 'NVIDIA' },
      shortSpec: 'GTX Series',
    };
  }
  // GT series (قدیمی‌ها)
  if (
    ...
    return {
      isMatch: true,
      confidence: 85,
      matchedModel: 'GT Series',
      specs: { tier: 'entry', brand: 'NVIDIA' },
      shortSpec: 'GT Series (قدیمی)',
    };
  }
  // RX 9000 (جدید)
  if (containsAny(n, ['9070', 'rx 9', 'radeon rx 9'])) {
    return {
      isMatch: true,
      confidence: 88,
      matchedModel: 'RX 9000 Series',
      specs: { tier: 'high', brand: 'AMD' },
      shortSpec: 'RX 9000 Series',
    };
  }
  // RX 7000
  if (containsAny(n, ['7900', '7800', '7700', '7600', 'radeon rx 7'])) {
    return {
      isMatch: true,
      confidence: 88,
      matchedModel: 'RX 7000 Series',
      specs: { tier: 'high', brand: 'AMD' },
      shortSpec: 'RX 7000 Series',
    };
  }
  // RX 6000
  if (containsAny(n, ['6950', '6900', '6800', '6700', '6600', '6500', '6400', 'radeon rx 6'])) {
    return {
      isMatch: true,
      confidence: 85,
      matchedModel: 'RX 6000 Series',
      specs: { tier: 'medium', brand: 'AMD' },
      shortSpec: 'RX 6000 Series',
    };
  }
  // Intel Arc
  if (containsAny(n, ['arc a', 'intel arc'])) {
    return {
      isMatch: true,
      confidence: 85,
      matchedModel: 'Intel Arc',
      specs: { tier: 'medium', brand: 'Intel' },
      shortSpec: 'Intel Arc',
    };
  }
...
export function detectMotherboard(name: string): PartDetection {
  ...
  if (containsAny(n, ['ddr5', ' ddr 5', ' d5 ', 'd5)', '(d5)', '/d5', '+d5', 'with d5']))
    ramType = 'DDR5';
  else if (containsAny(n, ['ddr4', ' ddr 4', ' d4 ', 'd4)', '(d4)', '/d4', '+d4', 'with d4']))
    ramType = 'DDR4';
  ...
  const ramSlotsMatch =
    n.match(/(?:ram slots?|memory slots?|dimm slots?|اسلات(?:‌| )?رم)\s*[:x×-]?\s*(\d)/i) ||
    n.match(/(\d)\s*(?:x|×)\s*(?:ddr4|ddr5|dimm|رم)/i);
  const m2SlotsMatch =
    n.match(/(?:m\.?2|nvme)\s*(?:slots?|اسلات)?\s*[:x×-]?\s*(\d)/i) ||
    n.match(/(\d)\s*(?:x|×)\s*(?:m\.?2|nvme)/i);
  const sataPortsMatch =
    n.match(/(?:sata)\s*(?:ports?|پورت)?\s*[:x×-]?\s*(\d)/i) || n.match(/(\d)\s*(?:x|×)\s*sata/i);
  const inferredRamSlots = formFactor === 'Mini-ITX' ? 2 : formFactor === 'Micro-ATX' ? 4 : 4;

  // ۶) تشخیص "مثبت"
  const hasPositive = containsAny(n, ['مادربرد', 'motherboard', 'mainboard', 'برد اصلی', 'm.b']);
  ...
  if (hasPositive) {
    return {
      ...
      shortSpec:
        `مادربرد ${socket || ''} ${ramType || ''} ${formFactor}${hasWifi ? ' WiFi' : ''}`.trim(),
    };
  }
...
export function detectRam(name: string): PartDetection {
  ...
  if (containsAny(n, ['ddr5', ' ddr 5'])) {
    specs.ramType = 'DDR5';
    specParts.push('DDR5');
  } else if (containsAny(n, ['ddr4', ' ddr 4'])) {
    specs.ramType = 'DDR4';
    specParts.push('DDR4');
  } else if (containsAny(n, ['ddr3', ' ddr 3'])) {
    specs.ramType = 'DDR3';
    specParts.push('DDR3');
  } else if (containsAny(n, ['ddr2'])) {
    specs.ramType = 'DDR2';
    specParts.push('DDR2');
  }
  ...
  if (freqFa) {
    ...
    if (freq >= 1600 && freq <= 8000) {
      specs.frequency = freq;
      specParts.push(`${freq}MHz`);
    }
  } else if (freqEn) {
  ...
  if (clMatch) {
    specs.casLatency = parseInt(clMatch[1]);
    specParts.push(`CL${clMatch[1]}`);
  }
  ...
  // Brand bonus
  const brandBonus = containsAny(n, [
    'kingston',
    'corsair',
    'gskill',
    'g.skill',
...
export function detectStorage(name: string): PartDetection {
  ...
  // Brand bonus
  const brandBonus = containsAny(n, ['samsung', 'سامسونگ'])
    ? 5
    : containsAny(n, ['wd', 'western digital'])
      ? 5
      : containsAny(n, ['crucial'])
        ? 5
        : containsAny(n, ['kingston'])
          ? 5
          : containsAny(n, ['seagate'])
            ? 4
            : containsAny(n, ['sabrent'])
              ? 4
              : containsAny(n, ['adata', 'xpg'])
                ? 3
                : containsAny(n, ['sk hynix', 'hynix'])
                  ? 4
                  : 0;
...
export function detectPsu(name: string): PartDetection {
  ...
  // Modularity
  if (containsAny(n, ['full modular', 'fully modular', 'ماژولار کامل', 'full modular']))
    specs.modular = 'full';
  ...
  // Brand bonus
  const brandBonus = containsAny(n, ['corsair', 'کورسیر'])
    ? 5
    : containsAny(n, ['seasonic'])
      ? 5
      : containsAny(n, ['be quiet'])
        ? 5
        : containsAny(n, ['coolermaster', 'cm storm'])
          ? 4
          : containsAny(n, ['fractal'])
            ? 4
            : containsAny(n, ['asus', 'rog'])
              ? 4
              : containsAny(n, ['msi'])
                ? 3
                : containsAny(n, ['gigabyte'])
                  ? 3
                  : containsAny(n, ['fsp', 'اف اس پی'])
                    ? 4
                    : containsAny(n, ['antec', 'آنتک'])
                      ? 3
                      : containsAny(n, ['aurest', 'اوست'])
                        ? 4
                        : containsAny(n, ['gamdias', 'گیمدیاس'])
                          ? 3
                          : containsAny(n, ['fater', 'فاطر'])
                            ? 3
                            : containsAny(n, ['deepcool', 'دیپ کول'])
                              ? 3
                              : 0;

  const positive =
    containsAny(n, ['پاور', 'psu', 'power supply', 'منبع تغذیه', 'power']) || specs.wattage;
...
export function detectCase(name: string): PartDetection {
  ...
  // Form Factor
  if (containsAny(n, ['full tower', 'فول تاور'])) {
    specs.formFactor = 'E-ATX';
    specParts.push('Full Tower');
  } else if (containsAny(n, ['mini-itx', 'mini itx', 'small form', 'sff'])) {
    specs.formFactor = 'Mini-ITX';
    specParts.push('Mini-ITX');
  } else if (containsAny(n, ['micro-atx', 'micro atx', 'matx'])) {
    specs.formFactor = 'Micro-ATX';
    specParts.push('Micro-ATX');
  } else if (containsAny(n, ['mid tower', 'mid-tower', 'midtower'])) {
    specs.formFactor = 'ATX';
    specParts.push('Mid Tower');
  } else if (containsAny(n, ['atx', 'tower'])) {
    specs.formFactor = 'ATX';
  }

  // Gaming / airflow / clearance hints
  const isGamingCase = containsAny(n, [
    'گیمینگ',
    'gaming',
    'mesh',
    'airflow',
  ...
  if (isGamingCase) {
    specs.gamingCase = true;
    specParts.push('Gaming');
  }
  if (containsAny(n, ['mesh', 'airflow', 'flow', 'تهویه'])) {
    specs.airflow = true;
    specParts.push('Airflow');
  }
  if (isOfficeCase) specs.officeCase = true;

  const gpuLenMatch =
    n.match(/(?:gpu|vga|کارت گرافیک|گرافیک).*?(\d{3})\s*(?:mm|میلی)/i) ||
  ...
  if (containsAny(n, ['360', 'سه فن', '3 fan', 'triple fan']))
    specs.radiatorSupport = Math.max(Number(specs.radiatorSupport || 0), 360);
  else if (containsAny(n, ['240', 'دو فن', '2 fan']))
    specs.radiatorSupport = Math.max(Number(specs.radiatorSupport || 0), 240);
  ...
  // Brand bonus
  const brandBonus = containsAny(n, ['lian li', 'لیان لی'])
    ? 5
    : containsAny(n, ['fractal'])
      ? 5
      : containsAny(n, ['nzxt'])
        ? 5
        : containsAny(n, ['corsair'])
          ? 5
          : containsAny(n, ['deepcool', 'دیپ کول'])
            ? 4
            : containsAny(n, ['coolermaster', 'کولر مستر'])
              ? 4
              : containsAny(n, ['asus', 'rog', 'ایسوس'])
                ? 4
                : containsAny(n, ['msi', 'ام اس آی'])
                  ? 3
                  : containsAny(n, ['gigabyte', 'گیگابایت', 'aorus'])
                    ? 3
                    : containsAny(n, ['phanteks'])
                      ? 4
                      : containsAny(n, ['be quiet'])
                        ? 4
                        : containsAny(n, ['aurest', 'اوست'])
                          ...
                          : containsAny(n, ['gamdias', 'گیمدیاس'])
                            ...
                            : containsAny(n, ['fater', 'فاطر'])
                              ? 3
                              : containsAny(n, ['logikey', 'لاجی کی'])
                                ? 3
                                : containsAny(n, ['sabit', 'سابیت'])
                                  ? 3
                                  : containsAny(n, ['dark flash', 'دارک فلش'])
                                    ? 3
                                    : containsAny(n, ['tsco', 'تسکو'])
                                      ? 3
                                      : 0;

  const positive = containsAny(n, ['کیس', 'case']);
...
export function detectCooler(name: string): PartDetection {
  ...
  // ۳) مثبت؟
  const positive =
    containsAny(n, ['کولر', 'خنک کننده', 'cooler', 'cpu cooler', 'fan heatsink']) || specs.brand;
...
export function detectCaseFan(name: string): PartDetection {
  ...
  // Color
  let color = '';
  if (containsAny(n, ['مشکی', 'black'])) {
    color = 'Black';
    specs.color = 'black';
  } else if (containsAny(n, ['سفید', 'white'])) {
    color = 'White';
    specs.color = 'white';
  }
  ...
  const brandBonus = containsAny(n, ['lian li', 'لیان لی'])
    ? 5
    : containsAny(n, ['corsair', 'کورسیر'])
      ? 5
      : containsAny(n, ['nzxt', 'ان زد ایکس تی'])
        ? 5
        : containsAny(n, ['deepcool', 'دیپ کول'])
          ? 4
          : containsAny(n, ['phanteks'])
            ...
            : containsAny(n, ['msi', 'ام اس آی'])
              ? 4
              : containsAny(n, ['coolermaster', 'کولر مستر', 'coolermastr'])
                ? 3
                : containsAny(n, ['aurest', 'اوست'])
                  ? 3
                  : containsAny(n, ['gamdias', 'گیمدیاس', 'گیم دیاس'])
                    ? 3
                    : containsAny(n, ['gamemax', 'گیم مکس'])
                      ? 3
                      : containsAny(n, ['logikey', 'لاجی کی'])
                        ? 3
                        : containsAny(n, ['magic', 'مجلیک', 'مجیک'])
                          ? 3
                          : containsAny(n, ['green', 'گرین'])
                            ? 3
                            : 0;

  const positive = containsAny(n, ['فن کیس', 'case fan', 'فن rgb', 'rgb fan', 'فن']) || specs.size;
...
export function detectPart(category: PartCategory, name: string): PartDetection {
  switch (category) {
    case 'cpu':
      return detectCpu(name);
    case 'gpu':
      return detectGpu(name);
    case 'motherboard':
      return detectMotherboard(name);
    case 'ram':
      return detectRam(name);
    case 'storage':
      return detectStorage(name);
    case 'psu':
      return detectPsu(name);
    case 'case':
      return detectCase(name);
    case 'cooler':
      return detectCooler(name);
    case 'case_fan':
      return detectCaseFan(name);
    case 'case_argb':
      return detectArgbStrip(name);
    default:
      return { isMatch: false, confidence: 0, specs: {}, shortSpec: '' };
  }
}
